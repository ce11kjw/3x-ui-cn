...
reset_webbasepath() {
    ...
    read -rp "确定要重置Web基本路径吗？(y/n)：" confirm
...
reset_config() {
    ...
    echo -e "全部 panel settings have been reset to default."
    restart
}
...
check_config() {
    ...
    if [[ -n "$existing_cert" ]]; then
        ...
        if [[ -n "$cert_sans" && $(echo "$cert_sans" | wc -l) -gt 1 ]]; then
            echo -e "${yellow}证书也覆盖：${plain} $(echo "$cert_sans" | grep -vx "$domain" | tr '\n' ' ')"
        fi
    else
        echo -e "${red}⚠ 警告：未配置SSL证书！${plain}"
        echo -e "${yellow}你可以为IP地址获取Let's Encrypt证书（有效期约6天，自动续期）。${plain}"
        read -rp "现在为IP生成SSL证书？[y/N]：" gen_ssl
        if [[ "$gen_ssl" == "y" || "$gen_ssl" == "Y" ]]; then
            ...
            if [[ $? -eq 0 ]]; then
                ...
                echo -e "${yellow}你可以通过主菜单选项20（SSL证书管理）重试。${plain}"
                start 0 > /dev/null 2>&1
            fi
        else
            echo -e "${yellow}访问地址：http://${server_ip}:${existing_port}${existing_webBasePath}${plain}"
            echo -e "${yellow}为安全起见，请使用主菜单选项20（SSL证书管理）配置SSL证书${plain}"
        fi
    fi
}
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        echo -e "端口已设置，请重启面板，并使用新端口 ${green}${port}${plain} 访问面板"
        confirm_restart
    fi
}
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        if [[ "${running_in_docker}" == "true" ]]; then
            LOGI "Docker中面板是容器主进程。"
            LOGI "要停止面板，请在主机上停止容器："
...
restart() {
    ...
    if [[ $? == 0 ]]; then
        ...
        LOGE "面板重启失败，可能启动时间超过两秒，请稍后查看日志"
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
restart_xray() {
    if [[ "${running_in_docker}" == "true" ]]; then
        if signal_xui USR1; then
            LOGI "Xray重启信号已发送成功，请检查日志确认重启是否成功lly"
        else
            LOGE "Could not find the running panel process to signal."
        fi
        sleep 2
...
enable() {
    if [[ "${running_in_docker}" == "true" ]]; then
        LOGI "开机自启由Docker重启策略控制（如docker-compose.yml中的restart: unless-stopped）。"
...
disable() {
    if [[ "${running_in_docker}" == "true" ]]; then
        LOGI "开机自启由Docker重启策略控制（如docker-compose.yml中的restart: unless-stopped）。"
        LOGI "在主机上设置容器restart: no以禁用开机自启。"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 0
...
show_status() {
    ...
    case $? in
        0)
            echo -e "Panel state: ${green}运行中${plain}"
            show_enable_status
            ;;
        1)
            echo -e "Panel state: ${yellow}未运行${plain}"
            show_enable_status
            ;;
        2)
...
show_xray_status() {
    check_xray_status
    if [[ $? == 0 ]]; then
        echo -e "xray state: ${green}运行中${plain}"
    else
...
open_ports() {
    ...
    read -rp "请输入要开放的端口（如80,443,2053或范围400-500）：" ports
...
delete_ports() {
    ...
    if [[ $choice -eq 1 ]]; then
        ...
        read -rp "请输入要删除的规则编号（1, 2等）：" rule_numbers
        ...
        # Deleting by ports
        read -rp "请输入要删除的端口（如80,443,2053或范围400-500）：" ports
...
ssl_cert_issue_main() {
    ...
    case "$choice" in
        ...
        4)
            ...
            ;;
        5)
            echo -e "${green}\t1.${plain} 使用/root/cert下的证书"
            echo -e "${green}\t2.${plain} 输入自定义证书路径（如certbot, /etc/letsencrypt/...）"
            ...
            ;;
        6)
            echo -e "${yellow}Let's Encrypt IP地址SSL证书${plain}"
...
ssl_cert_issue_for_ip() {
    ...
    LOGI "使用Let's Encrypt短期配置（约6天有效期，自动续期）"
    ...
    if [[ -n "$server_ip" ]]; then
        ...
        if ! confirm "${server_ip} 是否为此服务器的正确公网IPv4地址？" "y"; then
            server_ip=""
        fi
    else
    ...
    local WebPort=""
    read -rp "ACME HTTP-01监听端口（默认80）：" WebPort
    WebPort="${WebPort:-80}"
    ...
    if [[ "${WebPort}" -ne 80 ]]; then
        LOGI "提示：Let's Encrypt仍通过80端口连接，请将外网80端口转发至${WebPort} 进行验证。"
    fi
    ...
    while true; do
        if is_port_in_use "${WebPort}"; then
            ...
            local alt_port=""
            read -rp "请输入acme.sh独立监听的另一个端口（留空取消）：" alt_port
            alt_port="${alt_port// /}"
            if [[ -z "${alt_port}" ]]; then
    ...
    if [[ "$setPanel" == "y" || "$setPanel" == "Y" ]]; then
        if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then
            ...
            LOGI "  - 有效期：约6天（通过acme.sh定时任务自动续期）"
            echo -e "${green}访问地址：https://${server_ip}:${existing_port}${existing_webBasePath}${plain}"
...
ssl_cert_issue() {
    ...
    if ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
        ...
        if [[ -n "${acmeCertDir}" ]]; then
            ...
            LOGI "发现已有证书，域名：${domain}，将复用。"
            [[ -n "${certInfo}" ]] && LOGI "${certInfo}"
        else
            LOGW "发现acme.sh状态不完整，域名：${domain}（无有效证书文件）；正在清理并重新签发。"
    ...
    local WebPort=80
    read -rp "请选择使用的端口（默认80）：" WebPort
    if [[ -z ${WebPort} ]]; then
        WebPort=80
    elif [[ ! ${WebPort} =~ ^[1-9][0-9]*$ || ${WebPort} -gt 65535 ]]; then
        LOGE "你输入的端口 ${WebPort} 无效，将使用默认80端口。"
        WebPort=80
    fi
    ...
    LOGI "ACME默认reload命令为： ${yellow}x-ui restart"
    LOGI "此命令将在每次签发和续期证书时执行。"
    ...
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        ...
        case "$choice" in
            1)
                ...
                ;;
            2)
                LOGD "建议将x-ui restart放在末尾，这样即使其他服务失败也不会报错"
                read -rp "请输入自定义reload命令（如：systemctl reload nginx ; x-ui restart）：" reloadCmd
...
ssl_cert_issue_CF() {
    ...
    LOGI "****** 使用说明 ******"
    LOGI "请按以下步骤完成操作："
    LOGI "1. Cloudflare API Token（推荐，限定Zone:DNS:Edit范围）或全局API Key + 注册邮箱。"
    LOGI "2. 域名。"
    LOGI "3. 证书签发后，会提示你设置面板证书（可选）。"
    LOGI "4. 安装后脚本还支持SSL证书自动续期。"
    ...
    if [ $? -eq 0 ]; then
        ...
        CF_KeyType=""
        read -rp "你使用的是Cloudflare API Token还是全局API Key？(t/g) [默认t]：" CF_KeyType
    ...
    case "$choice" in
        ...
        3)
            ...
            if [[ $? == 0 ]]; then
                ...
                echo -e "${green}全部 users Unbanned successfully.${plain}"
                iplimit_main
            else
...
setup_fail2ban_iplimit() {
    ...
    if [[ -n "${XUI_ENABLE_FAIL2BAN+x}" && "${XUI_ENABLE_FAIL2BAN}" != "true" ]]; then
        echo -e "${yellow}XUI_ENABLE_FAIL2BAN=${XUI_ENABLE_FAIL2BAN}, 跳过Fail2ban配置。${plain}\n"
        return 0
    fi
...
remove_iplimit() {
    ...
    case "$num" in
        ...
        2)
            ...
            echo -e "${green}Fail2ban和IP限制已移除成功！${plain}\n"
            before_show_menu
            ;;
        0)
            show_menu
...
iplimit_remove_conflicts() {
    ...
    for file in "${jail_files[@]}"; do
        ...
        if test -f "${file}" && grep -qw '3x-ipl' ${file}; then
            sed -i "/\[3x-ipl\]/,/^$/d" ${file}
            echo -e "${yellow}正在移除jail中[3x-ipl]的冲突配置 (${file})!${plain}\n"
        fi
    done
}
...
pg_require_installed() {
    if ! postgresql_installed; then
        LOGE "PostgreSQL未安装。请先使用菜单选项1（安装PostgreSQL）。"
        return 1
    fi
}
...
pg_ensure_client() {
    ...
    echo -e "${yellow}正在安装PostgreSQL客户端工具（pg_dump/pg_restore）...${plain}" >&2
    case "${release}" in
        ubuntu | debian | armbian)
...
pg_upgrade_client() {
    ...
    if [[ -n "$have" ]]; then
        if [[ -z "$want" || "$have" -ge "$want" ]]; then
            LOGI "PostgreSQL客户端工具已安装（版本${have})."
            return 0
        fi
    ...
    LOGI "PostgreSQL客户端工具就绪（版本${have})."
    return 0
}
...
pg_install_server_action() {
    ...
    PG_LAST_DSN="$dsn"
    pg_ensure_client || LOGE "无法安装pg_dump/pg_restore（面板数据库备份可能不可用）。"
    echo ""
...
migrate_to_postgres() {
    ...
    if [[ -z "$dsn" ]]; then
        echo ""
        echo -e "${green}\t1.${plain} 本地安装PostgreSQL并创建专用用户/数据库（推荐）"
        echo -e "${green}\t2.${plain} 使用已有PostgreSQL服务器（输入DSN）"
        read -rp "请选择[1]：" pg_mode
        pg_mode="${pg_mode:-1}"
        if [[ "$pg_mode" == "2" ]]; then
            while [[ -z "$dsn" ]]; do
                read -rp "请输入PostgreSQL DSN（格式：postgres://用户名:密码@主机:端口/数据库名?sslmode=disable）：" dsn
                dsn="${dsn// /}"
            done
        else
            LOGI "正在本地安装PostgreSQL（可能需要一点时间）..."
            dsn=$(pg_install_local)
            if [[ $? -ne 0 || -z "$dsn" ]]; then
    ...
    pg_ensure_client || LOGE "无法安装pg_dump/pg_restore（面板内数据库备份/恢复可能不可用）。"
    ...
    pg_write_env "$dsn"
    LOGI "数据库设置已写入 $(xui_env_file_path) (XUI_DB_TYPE=postgres)."
    ...
    if check_status; then
        ...
        LOGE "面板未启动。请检查日志（主菜单选项17）。SQLite数据保持不变。"
    fi
}

postgresql_menu() {
    echo -e "${green}\t1.${plain} ${green}安装${plain} PostgreSQL (server + client + xui db)"
    echo -e "${green}\t2.${plain} 迁移SQLite ${green}->${plain} PostgreSQL"
    ...
    echo -e "${green}\t6.${plain} 重启PostgreSQL"
    echo -e "${green}\t7.${plain} ${green}Enable${plain} Autostart on boot"
    echo -e "${green}\t8.${plain} 查看PostgreSQL日志"
    echo -e "${green}\t9.${plain} 转换SQLite ${green}.db <-> .dump${plain}"
    ...
    case "$choice" in
        ...
        9)
            ...
            ;;
        10)
            read -rp "需要的PostgreSQL主版本号（空=任意）：" pg_client_ver
...
# Convert between the panel's SQLite database 和可移植 .dump (SQL text)
...
migrate_db() {
    ...
    if ! "$bin" migrate-db -h 2>&1 | grep -q -- '-dump'; then
        LOGE "此x-ui版本尚不支持.db <-> .dump转换。"
        LOGE "请先更新面板（x-ui update）到支持migrate-db的版本。"
        return 1
    fi
    ...
    if [[ "$mode" == "dump" ]]; then
        ...
        if [[ -f "$output" ]]; then
            confirm "输出文件 ${output} 已存在，将被覆盖。是否继续？" "n" || return 0
        fi
        ...
        if [[ "$output" == "$default_db" ]] && check_status > /dev/null 2>&1; then
            LOGE "拒绝恢复到正在使用的数据库（${default_db}) while x-ui is running."
            LOGE "请先停止面板（x-ui stop）或选择不同的输出路径。"
            return 1
        fi
        if [[ -f "$output" ]]; then
            confirm "输出文件 ${output} 已存在，将被覆盖。是否继续？" "n" || return 0
...
|  ${blue}x-ui restart-xray${plain}          - 重启Xray                     │
...
│────────────────────────────────────────────────│
│  ${green}20.${plain} SSL证书管理               │
│  ${green}21.${plain} Cloudflare SSL证书               │
│  ${green}22.${plain} IP限制管理                      │
│  ${green}23.${plain} 防火墙管理                      │
│  ${green}24.${plain} SSH端口转发管理           │
│  ${green}25.${plain} PostgreSQL管理                    │
│────────────────────────────────────────────────│
│  ${green}26.${plain} 启用BBR                               │
│  ${green}27.${plain} 更新Geo文件                         │
│  ${green}28.${plain} Ookla网速测试                       │
╚────────────────────────────────────────────────╝
"
    show_status
